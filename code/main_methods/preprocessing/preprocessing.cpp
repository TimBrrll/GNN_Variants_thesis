...
#else
#include </opt/homebrew/opt/pybind11/include/pybind11/pybind11.h>
#include </opt/homebrew/opt/pybind11/include/pybind11/stl.h>
#include </opt/homebrew/opt/pybind11/include/pybind11/eigen.h>
#endif
...
vector<vector<uint>> generate_full_multiset_2(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();

    unordered_map<Node, TwoTuple> node_to_multiset;
    unordered_map<TwoTuple, Node> multiset_to_node;

    Node num_multisets = 0;

    for (Node i = 0; i < num_nodes; ++i)
    {
        for (Node j = i; j < num_nodes; ++j)
        {
            node_to_multiset.insert({{num_multisets, make_tuple(i, j)}});
            multiset_to_node.insert({{make_tuple(i, j), num_multisets}});
            num_multisets++;
        }
    }
    vector<vector<uint>> nonzero_components;

    for (Node i = 0; i < num_multisets; ++i)
    {
        TwoTuple m = node_to_multiset.find(i)->second;
        Node m1 = std::get<0>(m);
        Node m2 = std::get<1>(m);

        for (Node j = 0; j < num_nodes; ++j)
        {
            unordered_map<TwoTuple, Node>::const_iterator v1 = multiset_to_node.find(make_tuple(m2, j));
            unordered_map<TwoTuple, Node>::const_iterator v2 = multiset_to_node.find(make_tuple(j, m2));

            unordered_map<TwoTuple, Node>::const_iterator v3 = multiset_to_node.find(make_tuple(m1, j));
            unordered_map<TwoTuple, Node>::const_iterator v4 = multiset_to_node.find(make_tuple(j, m1));

            if (v1 != multiset_to_node.end())
            {
                nonzero_components.push_back({{i, v1->second}});
            }
            else
            {
                nonzero_components.push_back({{i, v2->second}});
            }
            if (v3 != multiset_to_node.end())
            {
                nonzero_components.push_back({{i, v3->second}});
            }
            else
            {
                nonzero_components.push_back({{i, v4->second}});
            }
        }
    }
...
vector<vector<uint>> generate_local_sparse_multiset_2(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();
    ...
    for (Node i = 0; i < num_multisets; ++i)
    {
        ...
        Node m1 = std::get<0>(m);
        Node m2 = std::get<1>(m);
        Nodes m1_neighbors = g.get_neighbours(m1);
        Nodes m2_neighbors = g.get_neighbours(m2);

        for (Node w_n : m1_neighbors)
        {
            unordered_map<TwoTuple, Node>::const_iterator v1 = multiset_to_node.find(make_tuple(m2, w_n));
            unordered_map<TwoTuple, Node>::const_iterator v2 = multiset_to_node.find(make_tuple(w_n, m2));
        ...
        for (Node w_n : m2_neighbors)
        {

            unordered_map<TwoTuple, Node>::const_iterator v3 = multiset_to_node.find(make_tuple(m1, w_n));
            unordered_map<TwoTuple, Node>::const_iterator v4 = multiset_to_node.find(make_tuple(w_n, m1));
            if (v3 != multiset_to_node.end())
            {
                nonzero_components.push_back({{i, v3->second}});
            }
...
vector<vector<uint>> generate_local_sparse_multiset_3(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();

    // Maps node in two set graph to correponding two set.
    unordered_map<Node, ThreeTuple> node_to_three_multiset;
    unordered_map<ThreeTuple, Node> three_multiset_to_node;

    Node num_three_multisets = 0;

    for (Node i = 0; i < num_nodes; ++i)
    {
        for (Node j = i; j < num_nodes; ++j)
        {
            for (Node k = j; k < num_nodes; ++k)
            {
                node_to_three_multiset.insert({{num_three_multisets, make_tuple(i, j, k)}});
                three_multiset_to_node.insert({{make_tuple(i, j, k), num_three_multisets}});
                num_three_multisets++;
            }
        }
    }
    ...
    for (Node i = 0; i < num_three_multisets; ++i)
    {
        ThreeTuple m = node_to_three_multiset.find(i)->second;
        Node m1 = std::get<0>(m);
        Node m2 = std::get<1>(m);
        Node m3 = std::get<2>(m);
        Nodes m1_neighbors = g.get_neighbours(m1);
        Nodes m2_neighbors = g.get_neighbours(m2);
        Nodes m3_neighbors = g.get_neighbours(m3);

        for (Node w_n : m1_neighbors)
        {
            if (w_n < m2)
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(w_n, m2, m3));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m2, m3, w_n));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m2, w_n, m3));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
        ...
        for (Node w_n : m2_neighbors)
        {
            if (w_n < m1)
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(w_n, m1, m3));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m1, m3, w_n));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m1, w_n, m3));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
        ...
        for (Node w_n : m3_neighbors)
        {
            if (w_n < m1)
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(w_n, m1, m2));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m1, m2, w_n));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<ThreeTuple, Node>::const_iterator v1 = three_multiset_to_node.find(make_tuple(m1, w_n, m2));
                if (v1 != three_multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
...
vector<vector<uint>> generate_local_sparse_tupleset_3(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();
...
vector<vector<uint>> generate_all_tuplesets_2(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();

    unordered_map<Node, TwoTuple> node_to_tupleset;
    unordered_map<TwoTuple, Node> tupleset_to_node;

    Node num_tuplesets = 0;

    for (Node i = 0; i < num_nodes; ++i)
    {
        for (Node j = 0; j < num_nodes; ++j)
        {
            node_to_tupleset.insert({{num_tuplesets, make_tuple(i, j)}});
            tupleset_to_node.insert({{make_tuple(i, j), num_tuplesets}});
            num_tuplesets++;
        }
    }
    vector<vector<uint>> nonzero_components;

    for (Node i = 0; i < num_tuplesets; ++i)
    {
        TwoTuple m = node_to_tupleset.find(i)->second;
        Node m1 = std::get<0>(m);
        Node m2 = std::get<1>(m);
        ...
        for (Node j = 0; j < num_nodes; ++j)
        {
            ...
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = tupleset_to_node.find(make_tuple(j, m2));
                if (v1 != tupleset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            }
            ...
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = tupleset_to_node.find(make_tuple(m1, j));
                if (v1 != tupleset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
...
vector<vector<uint>> generate_local_sparse_tuplesets_2(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();
    ...
    for (Node i = 0; i < num_tuplesets; ++i)
    {
        ...
        for (Node w_n : m1_neighbors)
        {
            if (w_n != m1)
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = tupleset_to_node.find(make_tuple(w_n, m2));
                if (v1 != tupleset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
        ...
        for (Node v_n : m2_neighbors)
        {
            if (v_n != m2)
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = tupleset_to_node.find(make_tuple(m1, v_n));
                if (v1 != tupleset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
...
vector<vector<uint>> generate_local_sparse_set_2(const Graph &g)
{
    size_t num_nodes = g.get_num_nodes();
    ...
    for (Node i = 0; i < num_multisets; ++i)
    {
        ...
        for (Node w_n : m1_neighbors)
        {
            ...
            {
                nonzero_components.push_back({{w_n, m2}});
                unordered_map<TwoTuple, Node>::const_iterator v1 = multiset_to_node.find(make_tuple(w_n, m2));
                if (v1 != multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
        ...
        for (Node v_n : m2_neighbors)
        {
            if (v_n > m1)
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = multiset_to_node.find(make_tuple(m1, v_n));
                if (v1 != multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
            ...
            {
                unordered_map<TwoTuple, Node>::const_iterator v1 = multiset_to_node.find(make_tuple(v_n, m1));
                if (v1 != multiset_to_node.end())
                {
                    nonzero_components.push_back({{i, v1->second}});
                }
...
vector<unsigned long> get_multiset_node_labels_3(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    size_t num_nodes = g.get_num_nodes();
    // New graph to be generated.
...
vector<unsigned long> get_tupleset_node_labels_3(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    size_t num_nodes = g.get_num_nodes();
    // New graph to be generated.
...
// Generate node labels for two-tuple graph of graph g.
vector<unsigned long> get_node_labels_2(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
...
// Generate node labels for two-tuple graph of graph g.
vector<unsigned long> get_multiset_node_labels_2_2(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    // Get node and edge labels.
    Labels labels;
    vector<unsigned long> tuple_labels;
...
vector<unsigned long> get_tupleset_node_labels_2_2(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    // Get node and edge labels.
    Labels labels;
    vector<unsigned long> tuple_labels;
...
vector<unsigned long> get_set_node_labels_2_2(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    // Get node and edge labels.
    Labels labels;
    vector<unsigned long> tuple_labels;
...
vector<unsigned long> get_node_labels_multisets_2_1(const Graph &g, const bool use_labels, const bool use_edge_labels)
{
    // Get node and edge labels.
    Labels labels;
    vector<unsigned long> tuple_labels;
...
get_all_multisets_matrices_2(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_full_multiset_2(g));
    }
...
vector<vector<vector<uint>>>
get_local_sparse_multisets_matrices_2(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_local_sparse_multiset_2(g));
    }
...
vector<vector<vector<uint>>>
get_local_sparse_multiset_matrices_3(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_local_sparse_multiset_3(g));
    }
...
vector<vector<vector<uint>>>
get_local_sparse_tupleset_matrices_3(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
...
vector<vector<vector<uint>>>
get_all_tupleset_matrices_2(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_all_tuplesets_2(g));
    }
...
vector<vector<vector<uint>>>
get_local_sparse_tupleset_matrices_2(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_local_sparse_tuplesets_2(g));
    }
...
vector<vector<vector<uint>>>
get_local_sparse_set_2(string name, const std::vector<int> &indices)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb_new)
    {
        matrices.push_back(generate_local_sparse_set_2(g));
    }
...
vector<vector<unsigned long>> get_all_node_labels_2(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_node_labels_2(g, use_node_labels, use_edge_labels);
...
vector<vector<unsigned long>> get_all_multiset_node_labels_2_2(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    vector<vector<unsigned long>> node_labels;
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_multiset_node_labels_2_2(g, use_node_labels, use_edge_labels);
        vector<unsigned long> new_color;
...
vector<vector<unsigned long>> get_all_tupleset_node_labels_2_2(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    vector<vector<unsigned long>> node_labels;
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_tupleset_node_labels_2_2(g, use_node_labels, use_edge_labels);
        vector<unsigned long> new_color;
...
vector<vector<unsigned long>> get_all_set_node_labels_2_2(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    vector<vector<unsigned long>> node_labels;
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_set_node_labels_2_2(g, use_node_labels, use_edge_labels);
        vector<unsigned long> new_color;
...
vector<vector<unsigned long>> get_all_multiset_node_labels_3(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    vector<vector<unsigned long>> node_labels;
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_multiset_node_labels_3(g, use_node_labels, use_edge_labels);
        vector<unsigned long> new_color;
...
vector<vector<unsigned long>> get_all_tupleset_node_labels_3(string name, const bool use_node_labels, const bool use_edge_labels)
{
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(name);
    gdb.erase(gdb.begin() + 0);
    vector<vector<unsigned long>> node_labels;
    ...
    for (auto &g : gdb)
    {
        vector<unsigned long> colors = get_tupleset_node_labels_3(g, use_node_labels, use_edge_labels);
        vector<unsigned long> new_color;
...
PYBIND11_MODULE(preprocessing, m)
{
    ...
    m.def("get_all_multiset_matrices_2", &get_all_multisets_matrices_2);
    m.def("get_all_tupleset_matrices_2", &get_all_tupleset_matrices_2);
    m.def("get_local_sparse_multiset_matrices_2", &get_local_sparse_multisets_matrices_2);
    m.def("get_local_sparse_tupleset_matrices_2", &get_local_sparse_tupleset_matrices_2);
    m.def("get_local_sparse_set_2", &get_local_sparse_set_2);

    m.def("get_local_sparse_multiset_matrices_3", &get_local_sparse_multiset_matrices_3);
    m.def("get_local_sparse_tupleset_matrices_3", &get_local_sparse_tupleset_matrices_3);

    m.def("get_all_multiset_node_labels_2_2", &get_all_multiset_node_labels_2_2);
    m.def("get_all_tupleset_node_labels_2_2", &get_all_tupleset_node_labels_2_2);
    m.def("get_all_set_node_labels_2_2", &get_all_set_node_labels_2_2);
    m.def("get_all_multiset_node_labels_3", &get_all_multiset_node_labels_3);
    m.def("get_all_tupleset_node_labels_3", &get_all_tupleset_node_labels_3);
