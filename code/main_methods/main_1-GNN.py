...
from neural_models.OneGnn import OneGnn
    ...
    for i in range(repetitions):
        ...
        for train_index, test_index in kfold.split(range(len(dataset))):
            ...
            for hu in hidden_units:
                model = OneGnn(dataset=dataset, hidden_units=hu).to(device)
...
if __name__ == "__main__":
    ...
    dataset_name = ["ENZYMES", "IMDB-BINARY", "IMDB-MULTI", "PROTEINS", "PTC_FM"]
    batch_size = 32
    n_folds = 5
    ...
    for dataset in dataset_name:
        ...
        print("#####################################################")
        print(
            f"FINAL RESULT 1-GNN for {dataset}: mean_losses: {loss}, std_losses: {std}"
        )
        print("#####################################################")
        shutil.rmtree("../tmp")
