...
from neural_models.Gcn_Mean import Gcn_Mean
        ...
        for train_index, test_index in kfold.split(range(len(dataset))):
            ...
            for layer in layers:
                for hu in hidden_units:
                    model = Gcn_Mean(dataset=dataset, layers=layer, hidden_units=hu).to(
                        device
                    )
...
if __name__ == "__main__":
    ...
    for dataset in dataset_name:
        ...
        print(
            f"FINAL RESULT GCNMEAN for {dataset}: mean_losses: {loss}, std_losses: {std}"
        )
        shutil.rmtree("../tmp")
